...
%% almost as usual
\author{Jan Graffelman\\Universitat Polit\`ecnica de Catalunya}
...
\Abstract{
  ...
  \pkg{HardyWeinberg} package offers the classical tests for
  equilibrium, functions for power computation and for the simulation
  of marker data under equilibrium and disequilibrium. Functions 
...
The \pkg{HardyWeinberg} package~\citep{Graffel23} consists of a set of
...
% This paper describes the state of the art of version 1.5.3 of the package
% and substitutes the vignette that accompanied the previous version of the package.
...
equilibrium. Section~\ref{sec:graphics} treats graphical
...
coefficient of \Sexpr{round(HardyWeinberg:::ThetatoF(af(x),8),digits=4)}.
...
plotted in a ternary diagram in Figure~\ref{fig:simulated} in order to
...
\caption{Ternary plots for markers simulated under different conditions. (a) multinomial sampling with $p=0.5$. 
  ...
  (e) sampling from the Levene-Haldane distribution with fixed allele frequencies, (f) a data set in exact equilibrium with a uniform allele frequency. Red points represent markers that are significant in a chi-square test for HWE, green points represent non-significant markers.}\label{fig:simulated}
\end{center}
\end{figure}
\clearpage
