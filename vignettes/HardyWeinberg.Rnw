...
%% almost as usual
\author{Jan Graffelman\\Universitat Polit\`ecnica de Catalunya\\version 1.5.6\\ \today}
...
\Abstract{
  ...
  tests for X-chromosomal markers have been developed and included in the package. Functions 
...
This paper describes the state of the art of version 1.5.5 of the package. 
...
the argument \code{x.linked=TRUE} to the function call. For a detailed treatment of the X-chromosomal 
tests, see~\citet{Graffel28}. The X-chromosomal procedures are omnibus tests that simultaneously test equality of allele frequencies
in males and females and Hardy-Weinberg proportions in females. Examples of testing X-chromosomal markers for HWE are given below in Section~\ref{sec:session}.
...
Finally, a summary of all X-chromosomal tests is obtained by
...
X-linked markers via the \code{x.linked} argument. 
